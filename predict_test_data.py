...
if __name__ == '__main__':
    ...
    im_folders = [image_folder_ok_test, image_folder_nok_test]
    for i_f in im_folders:

        print('doing folder ' + i_f)

        if i_f == image_folder_ok_test:
            ground_truth_str = pred_ok_str
        elif i_f == image_folder_nok_test:
            ground_truth_str = pred_nok_str

        for ctr, img in enumerate(os.listdir(i_f)):
            print('done ' + str(ctr + 1) + ' images')

            prepare_and_save_plot(i_f, height, width, model, img, results_test_folder, ground_truth_str, pred_ok_str,
                                  pred_nok_str)
